...
- RPM 安装的适配器现在可以校验并使用分散在已声明子包中的文件，包括适配器文件和共享
  skills。缺失的软件包或无法校验的文件仍会阻止启用，避免接受未经验证的输入
  ([#3417](https://github.com/agentic-os-org/ANOLISA/pull/3417))。
- RPM 事务和升级预检错误现在会对原生包管理器诊断中的仓库凭据、路径和查询参数进行脱敏，
  包括含空白字符的已配置 URL。保留 HTTP(S) 源站信息用于排查问题
