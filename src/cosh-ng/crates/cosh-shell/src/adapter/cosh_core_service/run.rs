...
pub(super) fn run_turn(
    ...
    if command.cancelled.load(Ordering::SeqCst) {
        // A cancelled run must report cancellation, not success. Strip any
        // AgentCompleted — whether it is the parser's synthetic fallback or a
        // genuine result that raced with the cancel signal — while preserving a
        // real AgentFailed so structured session failures are not lost.
        terminal_events.retain(|event| !matches!(event, AgentEvent::AgentCompleted { .. }));
        if !terminal_events
            .iter()
