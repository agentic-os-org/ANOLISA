impl TaskCoordinator {
    ...
    fn snapshot_preview(
        ...
        for change in &provider.changes {
            bytes += serde_json::to_vec(change)?.len() + 1;
            if bytes > MAX_GATEWAY_FRAME_BYTES / 2 {
                break;
            }
            shown += 1;
        }
        let changes_omitted = provider.changes.len() - shown;
        provider.changes.truncate(shown);
