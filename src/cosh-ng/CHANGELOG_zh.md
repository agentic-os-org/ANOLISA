...
### 新增
- cosh-shell 中的托管任务现已贯通端到端流程：可提交持久化的 Core 或 Codex 任务、选择 checkpoint 策略、断线重连后查看进度，并通过 Gateway 受控的恢复流程安全切换任务快照 (#2911)

### 修复
- 修复 /proc、/dev、/sys 等受保护路径的字面路径穿越拼写（如 /../proc/version）可绕过 readonly 自动执行拦截的问题：路径先做词法归一化再匹配，合法路径不受影响 (#2708)
- 修复带引号的 /dev/null 重定向目标（如 2>"/dev/null"）被误标为文件写入、导致审批卡片理由显示错误的问题：现在按空重定向分类，与无引号行为一致 (#2710)
