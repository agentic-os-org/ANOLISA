...
fn enforcement_blocks_unlink() {
    ...
    // elsewhere and the test would pass without exercising the policy.
    let fixture_dir = Path::new("/tmp/agentsight-enforce-e2e");
    let _ = std::fs::remove_dir_all(fixture_dir);
    std::fs::create_dir_all(fixture_dir).expect("create fixture dir");
    let test_file = fixture_dir.join(format!("target-{}.txt", Uuid::new_v4()));
    ...
    while Instant::now() < deadline {
        while let Ok(event) = violations.try_recv() {
            if event.binding_id == binding_id && event.operation.contains("unlink") {
                found = Some(event);
                break;
            }
        }
