...
#      predated the run.
#  36. Hermes registration state is read structurally, so a normal plugins.disabled
#      entry (what a successful disable writes) is a success, not a permanent
#      failure — while enabled, in either YAML shape, still fails.
...
assert_contains "says the registration cannot be confirmed removed" "$RUN_OUTPUT" \
...
assert_eq "an executable CLI that refuses is not a confirmed deregistration" \
  "$RUN_STATUS" "1"
# With the config read structurally this is now the stronger answer: the entry is
# provably still under plugins.enabled, not merely "unconfirmed".
assert_contains "says the plugin is still enabled after the CLI ran" "$RUN_OUTPUT" \
  "still lists tokenless under plugins.enabled"
assert_file "keeps the hermes adapter resources" \
...
# =============================================================================
# Scenario 55 — hermes registration state is read structurally
# =============================================================================
# "Does the file mention tokenless?" is not the question. A successful
# `plugins disable` moves the name into plugins.disabled — Hermes' documented
# contract — and a coarse text test read that as "still registered", so a
# *successful* uninstall failed forever: re-running the disable/remove its own
# error message suggested cannot clear a normal disabled entry. A shape-specific
# regex has the mirror flaw, missing a legal flow sequence.
...
hermes_state_case() {  # <label> <config|NONE> <cli> <expected-exit>
  local label="$1" cfg="$2" cli="$3" want="$4"
  local h="$S55_DIR/$label"
  rm -rf "$h"; mkdir -p "$h/home" "$h/hhome"
  if [ "$cfg" != "NONE" ]; then printf '%b' "$cfg" > "$h/hhome/config.yaml"; fi
  local st=0
  env -i PATH=/usr/bin:/bin HOME="$h/home" HERMES_HOME="$h/hhome" HERMES_BIN="$cli" \
    bash "$TOKENLESS_ROOT/adapters/tokenless/hermes/scripts/uninstall.sh" >/dev/null 2>&1 || st=$?
  assert_eq "hermes: $label" "$st" "$want"
}
# The success path, which the coarse test used to fail forever.
hermes_state_case disable-succeeds 'plugins:\n  enabled: [tokenless]\n' "$S55_GOOD" 0
# Still registered, in both legal YAML shapes, with a CLI that refuses.
hermes_state_case flow-enabled-refused 'plugins:\n  enabled: [tokenless]\n' /bin/false 1
hermes_state_case block-enabled-refused 'plugins:\n  enabled:\n    - tokenless\n' /bin/false 1
# Mentions that are not registrations.
hermes_state_case comment-mention '# tokenless was enabled here once\nplugins:\n  enabled: []\n' /bin/false 0
hermes_state_case unrelated-key 'other: tokenless\nplugins:\n  enabled: []\n' /bin/false 0
hermes_state_case block-disabled 'plugins:\n  enabled: []\n  disabled:\n    - tokenless\n' /bin/false 0
hermes_state_case flow-disabled 'plugins:\n  enabled: []\n  disabled: [tokenless]\n' /bin/false 0
hermes_state_case no-plugins-block 'logging:\n  level: info\n' /bin/false 0
# Cannot confirm.
hermes_state_case unparsable-enabled 'plugins:\n  enabled: |\n' /bin/false 1
hermes_state_case no-config-dead-cli NONE /bin/false 1
# No over-blocking: no CLI and no config means nothing was ever registered here.
hermes_state_case no-config-no-cli NONE "$S55_ABSENT" 0

# End to end: a config left in the normal "disabled" shape must let the whole
# receipt-driven uninstall finish, which is what the coarse test blocked forever.
run_script "$INSTALL_SH" hermes-disabled-ok "TOKENLESS_VERSION=$FAKE_VERSION"
assert_eq "install exits 0" "$RUN_STATUS" "0"
S55_ADAPTERS="$TEST_DIR/hermes-disabled-ok/home/.local/share/anolisa/adapters/tokenless"
S55_HOME="$TEST_DIR/hermes-disabled-ok/home"
rm -rf "${S55_ADAPTERS:?}/hermes"
cp -R "$TOKENLESS_ROOT/adapters/tokenless/hermes" "$S55_ADAPTERS/hermes"
mkdir -p "$S55_HOME/.hermes"
printf 'plugins:\n  enabled: []\n  disabled: [tokenless]\n' > "$S55_HOME/.hermes/config.yaml"
run_script "$UNINSTALL_SH" hermes-disabled-ok \
  "HERMES_BIN=$S55_ABSENT" "CLAUDE_BIN=$S55_ABSENT"
...
assert_no_file "and so is the receipt" "$(receipt_of hermes-disabled-ok)"
