...
def main():
    ...
    summary = {
        ...
        # This is intentionally stricter than the legacy published ratio, which
        # counted either a full path or a basename hit. Common basenames produce
        # false positives, so the strict headline uses full paths only and keeps
        # basename-only hits in a separate per-instance field.
        "named_definition": "full gold path only; basename-only hits reported separately",
    }
