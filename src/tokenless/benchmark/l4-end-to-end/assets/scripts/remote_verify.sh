...
if ! gjson="$(openclaw plugins list --json 2>/dev/null)"; then
    fail "cannot read plugin state as JSON"
    gjson=""
fi
for p in tokenless headroom; do
    # Parse the machine-readable listing rather than grepping the human table:
    # strings such as "enabled: false", a column header, or a dependency
    # description can all contain the word enabled and invert a text-based check.
    state=$(printf '%s' "$gjson" | python3 -c '
import json, sys
...
if not hits:
    print("missing")
elif any(item.get("enabled") is True for item in hits):
    print("enabled")
elif all(item.get("enabled") is False for item in hits):
    print("disabled")
else:
    print("unknown")
' "$p")
    case "$state" in
        disabled) pass "$p is installed and globally disabled" ;;
        enabled)  fail "$p is globally ENABLED; it would contaminate the baseline arm" ;;
        missing)  fail "$p is not installed; its arm cannot run" ;;
        *)        fail "$p state is not machine-readable ($state)" ;;
    esac
done
