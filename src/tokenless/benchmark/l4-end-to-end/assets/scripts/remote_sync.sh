...
RSYNC_OPTS=(--no-owner --no-group --partial)
ANOLISA_EXCLUDES=(
    --exclude target
    --exclude .venv
    --exclude node_modules
    --exclude /src/tokenless/third_party/rtk
    # Only the OpenClaw bundle is a gitignored build artefact. Anchor the pattern
    # to the transfer root: a bare `dist` also removes tracked files such as
    # src/tokenless/adapters/tokenless/dsh/dist/index.js, which build-dsh-plugin
    # validates but cannot regenerate on a fresh host.
    --exclude /src/tokenless/adapters/tokenless/openclaw/dist
)
HEADROOM_EXCLUDES=(
    --exclude target
    --exclude .venv
    --exclude node_modules
    # remote_setup.sh always rebuilds this bundle from the synced source.
    --exclude /plugins/openclaw/dist
)
...
# Dotfiles are excluded so a stray .DS_Store from the macOS source tree does not
# inflate the count and permanently trip analyze_runs.py's corpus_size_mismatch
# check against the audit's gold-resolvable count.
PROMPTS_COUNT="$(find "$PROMPTS_DIR" -maxdepth 1 -type f -not -name '.*' | wc -l | tr -d ' ')"
...
# Send values over stdin instead of interpolating the branch into a quoted
# remote command: git permits apostrophes in branch names, which would break the
# shell quoting and corrupt (or inject into) the provenance write.
printf 'corpus_rev=%s\ncorpus_branch=%s\ncorpus_files=%s\n' \
    "$PROMPTS_REV" "$PROMPTS_BRANCH" "$PROMPTS_COUNT" | \
    ${SSH_PW[@]+"${SSH_PW[@]}"} ssh "${SSH_OPTS[@]}" "$L4_SSH_USER@$L4_SSH_HOST" \
        "cat > '$L4_REMOTE_WORK/prompts.provenance'"
