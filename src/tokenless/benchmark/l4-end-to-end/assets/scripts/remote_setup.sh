...
#
# NETWORK: upstreams are reached directly. This reverses the previous revision,
# which routed every fetch through a mirror because nodejs.org,
# static.rust-lang.org and huggingface.co were unreachable from an earlier
# evaluation host and registry.npmjs.org timed out mid-tarball there. From these
# hosts all
# four answer directly, so the indirection is gone wherever it changed *what*
# gets installed, and kept only where it merely changes transfer speed (pypi,
# crates.io). The distinction matters for one specific reason recorded at step 3.
#
...
# Always rebuild. dist/ is excluded from the sync (a gitignored build artefact),
# and even a dist/ left by a prior run may predate the source remote_sync.sh just
# pushed. The build is fast and has its own log; the old file-exists guard is
# exactly what let a stale plugin be deployed and measured as the new one.
...
# Record the exact bytes deployed so the report can tie numbers to a bundle.
sha256sum adapters/tokenless/openclaw/dist/index.js > "$LOG/tokenless_dist.sha256" 2>&1 || true
...
sha256sum dist/index.js > "$LOG/headroom_dist.sha256" 2>&1 || true
