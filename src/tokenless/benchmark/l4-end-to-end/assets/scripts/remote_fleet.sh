...
if [ -n "${L4_DASHSCOPE_KEYS:-}" ]; then
    saved_ifs=$IFS
    IFS=,
    read -ra DASHSCOPE_KEYS <<< "$L4_DASHSCOPE_KEYS"
    IFS=$saved_ifs
...
run_parallel() {
    ...
    # Keep the current logs for diagnosis, but bound accumulation by pruning only
    # this harness's same-phase directories after seven days.
    find "${TMPDIR:-/tmp}" -maxdepth 1 -type d -name "l4-fleet-$label.*" \
