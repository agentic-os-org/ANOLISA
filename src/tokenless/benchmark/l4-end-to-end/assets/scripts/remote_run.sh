...
# Read the actual model from the baseline config used by this sweep rather than
# duplicating its id in the script. The value is recorded in sweep-summary.txt,
# used by both the drift sentinel and proxy prewarm, and later becomes report
# provenance; changing the config therefore cannot leave a stale claimed alias.
#
# The paired comparison requires every arm to run the same model, and this one
# alias stands in for all of them, so assert that every arm config agrees before
# trusting it: a diverging arm would otherwise be prewarmed against, and reported
# as, arm A's model while actually running its own.
MODEL_ALIAS=$(python3 - "$CFGDIR"/arm-*.json <<'PY'
import json, sys
primaries = set()
for path in sys.argv[1:]:
    primaries.add(json.load(open(path))["agents"]["defaults"]["model"]["primary"])
if len(primaries) != 1:
    sys.stderr.write(f"arm configs disagree on model.primary: {sorted(primaries)}\n")
    sys.exit(1)
print(next(iter(primaries)).split("/", 1)[-1])
PY
) || die "arm configs missing or disagree on the model alias"
[ -n "$MODEL_ALIAS" ] || die "arm configs have an empty model alias"
...
record_model_created() {
    ...
    # Feed the auth header through curl's stdin config so the provider key does
    # not appear in /proc/<pid>/cmdline (it already lives in the environment).
    printf 'header = "Authorization: Bearer %s"\n' "$DASHSCOPE_API_KEY" | \
        curl -s --max-time 20 --config - \
            https://dashscope.aliyuncs.com/compatible-mode/v1/models > "$out" 2>&1
    local created
    created=$(python3 - "$out" "$MODEL_ALIAS" <<'PY'
import json, sys
try:
    ...
    sys.exit(1)
alias = sys.argv[2]
hit = [m for m in doc.get("data", []) if m.get("id") == alias]
...
start_proxy() {
    ...
    for try in $(seq 1 "$launch_tries"); do
        ...
        if [ "$endpoints_ready" = yes ]; then
            local warm_body
            warm_body="{\"model\":\"$MODEL_ALIAS\",\"max_tokens\":1,\"temperature\":0,"
            warm_body+='"messages":[{"role":"user","content":"reply ok"}]}'
            for _ in 1 2 3; do
                if printf 'header = "Authorization: Bearer %s"\n' "$DASHSCOPE_API_KEY" | \
                    curl -sf -o /dev/null -m 120 --config - \
