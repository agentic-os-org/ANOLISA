...
def prompt_guidance(runs, hosts_map):
    ...
    for host, hdir in hosts_map.items():
        ...
        if os.path.exists(p):
            try:
                audits[host] = json.load(open(p))
            except (OSError, json.JSONDecodeError) as error:
                raise SystemExit(f"{host}: unreadable prompt disclosure audit: {error}")
        pp = f"{runs}/{hdir}/prompts.provenance"
        if os.path.exists(pp):
            values = dict(line.strip().split("=", 1) for line in open(pp) if "=" in line)
            try:
                corpus_files[host] = int(values["corpus_files"])
            except (KeyError, ValueError):
                corpus_files[host] = None

    if not audits:
        disclosure = {
            "audited": False,
            "note": "run audit_prompt_disclosure.py on every benchmark host and "
                    "sync prompt_disclosure_audit.json into each run tree",
        }
        named = {
            "audited": False,
        ...
        # `in_shard` legitimately differs by host; every other per-prompt fact
        # describes the shared corpus and must agree. Compare canonical rows
        # across every available host rather than accepting the first file found.
        canonical = {}
        for host, doc in audits.items():
            rows = []
            for row in doc.get("instances", []):
                rows.append({k: v for k, v in row.items() if k != "in_shard"})
            canonical[host] = json.dumps(sorted(rows, key=lambda r: r.get("instance_id", "")),
                                         sort_keys=True)
        # Cross-host consistency is undefined with a single audit: a one-element
        # set is vacuously "equal", so only claim agreement once at least two
        # hosts have synced an audit back (null, not True, in between).
        consistent = None if len(audits) < 2 else (len(set(canonical.values())) == 1)
        complete = set(audits) == set(hosts_map)
        methods = {doc.get("summary", {}).get("method") for doc in audits.values()}
        methods.discard(None)
        if len(methods) > 1:
            consistent = False

        counts = {host: len(doc.get("instances", [])) for host, doc in audits.items()}
        comparable_hosts = {
            host for host in counts
            if host in corpus_files and corpus_files[host] is not None
        }
        mismatches = {
            host: {"audit_prompts": counts[host],
                   "provenance_corpus_files": corpus_files[host]}
            for host in comparable_hosts
            if corpus_files[host] != counts[host]
        }
        size_mismatch = bool(mismatches) if comparable_hosts else None
        first = next(iter(audits.values()))
        disclosure = {
            "audited": True,
            "campaign_hosts_complete": complete,
            "consistent_across_hosts": consistent,
            "method": next(iter(methods)) if len(methods) == 1 else sorted(methods),
            "measured_shard_by_host": {
                host: doc.get("summary", {}).get("measured_shard")
                for host, doc in audits.items()
            },
            "all_corpus": first.get("summary", {}).get("all_corpus") if consistent else None,
            "audited_prompts_by_host": counts,
            "corpus_files_by_host": corpus_files,
            "corpus_size_mismatch": size_mismatch,
            "corpus_size_mismatches": mismatches,
            # The two counts use different definitions, so a mismatch of N usually
            # means N stray non-prompt files in the synced dir, not a swapped
            # corpus. Spelled out so the flag stays interpretable.
            "corpus_files_definition": "corpus_files counts every regular file in "
                                       "the synced prompts dir (dotfiles excluded); "
                                       "audit_prompts counts only ids that resolve "
                                       "to a SWE-bench gold patch",
        }

        if consistent:
            hist = {"0": 0, "1": 0, "2": 0, "3+": 0}
            for row in first.get("instances", []):
                count = row.get("gold_files_named") or 0
                hist["3+" if count >= 3 else str(count)] += 1
            named = {
                "audited": True,
                "campaign_hosts_complete": complete,
                "consistent_across_hosts": consistent,
                "source": "derived from prompt_disclosure_audit.json files that "
                          "agree across every host",
                "definition": first.get("summary", {}).get("named_definition"),
                "corpus_prompts": len(first.get("instances", [])),
            ...
            named = {
                ...
                "campaign_hosts_complete": complete,
                "consistent_across_hosts": consistent,
                "corpus_prompts_by_host": counts,
                "prompts_by_files_named": None,
                "note": (
                    "fewer than two host audits synced back; cross-host "
                    "consistency is undefined, so no campaign-wide histogram "
                    "is emitted"
                    if consistent is None else
                    "host audit files disagree; no campaign-wide histogram emitted"
                ),
            }
...
def run_integrity(runs, hosts_map):
    """Per-arm timing, because the arms were not one contiguous sweep on
...
def model_aliases(runs, hosts_map):
    """Distinct model aliases each host's sweep recorded, in first-seen order.

    A single-attempt sweep records one; a host re-run against a different alias
    records several in the append log. Returning the whole sequence -- not just
    the last line -- lets provenance and the gates surface an intra-host
    divergence instead of silently adopting whichever attempt wrote last. Hosts
    with no recorded alias are omitted so a missing reading reads as incomplete.
    """
    out = {}
    for host, hdir in hosts_map.items():
        path = f"{runs}/{hdir}/out/repeat-{REPEAT}/sweep-summary.txt"
        ...
        if os.path.exists(path):
            for line in open(path, errors="ignore"):
                match = re.search(r"model_alias=(\S+)", line)
...
def gates(runs, hosts_map, aliases):
    """Validity gates whose failure voids a run rather than degrading it."""
    out = {}
    for host, hdir in hosts_map.items():
        host_aliases = aliases.get(host) or []
        alias_field = host_aliases[0] if len(host_aliases) == 1 else (host_aliases or None)
        h = {"model_alias": alias_field}
        ...
        for when in ("before", "after"):
            ...
            if not os.path.exists(mf):
                continue
            entries = json.load(open(mf)).get("data", [])
            matched = [entry for entry in entries if entry.get("id") in set(host_aliases)]
            h[f"model_created_{when}"] = matched[0].get("created") if matched else None
...
def provenance(runs, hosts_map, aliases):
    """Provenance read from the run tree, never hardcoded.

    Any field that cannot be recovered from every host is left null (or a list
    when hosts disagree) and ``incomplete`` is set, so a partial run tree cannot
    inherit identity from an earlier campaign.
    """

    def collapse(per_host):
        # per_host maps a host to the distinct values seen in its append log. A
        # value is complete only when every host reported exactly one and they
        # all agree; an intra-host divergence (a host with >1 value from a
        # re-run) or a cross-host divergence both surface as the sorted list and
        # force incomplete, so the report never claims one identity for a tree
        # that actually holds several.
        all_values = sorted({v for values in per_host.values() for v in values})
        every_present = set(per_host) == set(hosts_map) and all(per_host.get(h) for h in hosts_map)
        every_single = bool(per_host) and all(len(values) == 1 for values in per_host.values())
        complete = every_present and every_single and len(all_values) == 1
        value = all_values[0] if len(all_values) == 1 else (all_values or None)
        return value, complete

    def add(store, host, value):
        if value:
            seq = store.setdefault(host, [])
            if value not in seq:
                seq.append(value)

    revisions = {}
    plugin_versions = {"tokenless": {}, "headroom": {}}
    bundle_hashes = {
        "tokenless_bundle_sha256": ("logs/tokenless_dist.sha256", {}),
        "reference_bundle_sha256": ("logs/headroom_dist.sha256", {}),
    }
    for host, hdir in hosts_map.items():
        summary = f"{runs}/{hdir}/out/repeat-{REPEAT}/sweep-summary.txt"
        if os.path.exists(summary):
            for line in open(summary, errors="ignore"):
                match = re.search(r"revision=(\S+)", line)
                if match and match.group(1) != "unknown":
                    add(revisions, host, match.group(1))

        plugins = f"{runs}/{hdir}/logs/plugins_list.log"
        if os.path.exists(plugins):
            for line in open(plugins, errors="ignore"):
                ...
                for name in plugin_versions:
                    if name in low:
                        add(plugin_versions[name], host, version.group(1))

        for _, (rel, store) in bundle_hashes.items():
            path = f"{runs}/{hdir}/{rel}"
            if os.path.exists(path):
                text = open(path).read().strip().split()
                # Accept only a well-formed digest. A failed sha256sum can leave
                # its error text in the file; a fabricated fingerprint is worse
                # than a null, so a non-hash is dropped and reads as incomplete.
                if text and re.fullmatch(r"[0-9a-f]{64}", text[0]):
                    add(store, host, text[0])

    anolisa_commit, revision_complete = collapse(revisions)
    tokenless_plugin, tokenless_version_complete = collapse(plugin_versions["tokenless"])
    reference_plugin, reference_version_complete = collapse(plugin_versions["headroom"])
    model_alias, alias_complete = collapse(aliases)
    tokenless_hash, tokenless_hash_complete = collapse(bundle_hashes["tokenless_bundle_sha256"][1])
    reference_hash, reference_hash_complete = collapse(bundle_hashes["reference_bundle_sha256"][1])

    prov = {
        "anolisa_commit": anolisa_commit,
        "tokenless_plugin": tokenless_plugin,
        "reference_plugin": reference_plugin,
        "tokenless_bundle_sha256": tokenless_hash,
        "reference_bundle_sha256": reference_hash,
        "model_alias": model_alias,
    }
    prov["incomplete"] = not all((
        revision_complete,
        tokenless_version_complete,
        reference_version_complete,
        tokenless_hash_complete,
...
def main():
    ...
    if discovered:
        ARMS = discovered
    unknown = [a for a in ARMS if a not in DESIGN["arm_definitions"]]
    if unknown:
        raise SystemExit(f"run tree contains unknown arms: {unknown}")
    if "a" not in ARMS:
        raise SystemExit("run tree has no baseline arm a; paired comparisons are undefined")
    for arm, definition in DESIGN["arm_definitions"].items():
        definition["executed"] = arm in ARMS
    DESIGN["sharding"]["arms_per_host"] = ARMS
    targets = [a for a in ARMS if a != "a"]
    clean = guard_clean(args.runs, hosts_map, REPEAT)
    aliases = model_aliases(args.runs, hosts_map)
    prov = provenance(args.runs, hosts_map, aliases)
    ...
    doc = {
        ...
        "validity_gates": gates(args.runs, hosts_map, aliases),
