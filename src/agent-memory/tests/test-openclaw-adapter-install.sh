...
# vacates the slot while config still says memory-core is disabled. `none` is
# deliberately not in this list: it is an operator choice, pinned below.
for owner in memory-anolisa '"memory-anolisa"' memory-core MEMORY-CORE \
        null undefined '(empty)' absent ''; do
    export TEST_MEMORY_SLOT="$owner"
    touch -- "$MEMORY_CORE_MARKER"
    run_uninstall
    [ "$(enable_calls)" = 1 ] \
...
# --- slots.memory=none is the operator's, not an unset slot (#3222) ----------
# OpenClaw documents plugins.slots.memory = "none" as an explicit "disable
# memory plugins" value (https://docs.openclaw.ai/gateway/config-extensions),
# not as a rendering of "no owner". A real host agrees: with the slot on "none",
# `plugins enable memory-core` prints `Exclusive slot "memory" switched from
# "none" to "memory-core".` and flips plugins.entries.memory-core.enabled back
# to true — so a marker-driven restore here silently undoes the operator's
# choice to run with no memory backend, which is the same undo the third-owner
# guard exists to prevent. The probe must classify every rendering of "none"
# the same way, or the choice survives on some hosts and not others.
rm -f -- "$OPENCLAW_CFG_UNDER_TEST"
for owner in none '"none"' NONE 'plugins.slots.memory=none' 'slots.memory: none'; do
    export TEST_MEMORY_SLOT="$owner"
    touch -- "$MEMORY_CORE_MARKER"
    run_uninstall
    [ "$(enable_calls)" = 0 ] \
        || fail "slot owner '${owner}' re-enabled a memory-core the operator disabled via 'none'"
    [ "$(slot_probe_calls)" = 1 ] \
        || fail "slot owner '${owner}': uninstall never asked who owns the memory slot"
    [ -f "$MEMORY_CORE_MARKER" ] \
        || fail "slot owner '${owner}' dropped the record of a memory-core it left disabled"
    expect_log 'Left memory-core disabled'
    expect_log "explicit choice to run with no memory plugin"
    expect_no_log 'Re-enabled memory-core'
done
unset TEST_MEMORY_SLOT
rm -f -- "$MEMORY_CORE_MARKER"
echo 'PASS: an explicit slots.memory=none survives the uninstall'

# The persisted config is the fallback source of truth for the same reason it is
# for a third-party owner: a host whose `config get` cannot answer still carries
# the operator's "none" in openclaw.json, and misreading it there would re-enable
# memory-core anyway.
if command -v jq &>/dev/null || command -v python3 &>/dev/null; then
    cat > "$OPENCLAW_CFG_UNDER_TEST" <<'JSON'
{
  "plugins": {
    "allow": ["memory-anolisa"],
    "entries": {
      "memory-anolisa": { "enabled": true },
      "memory-core": { "enabled": false }
    },
...
JSON
    ...
    run_uninstall
    [ "$(enable_calls)" = 0 ] \
        || fail 'a slots.memory=none only openclaw.json knows about did not stop the restore'
    grep -q '"memory": "none"' "$OPENCLAW_CFG_UNDER_TEST" \
        || fail 'the persisted memory slot no longer reads none'
    [ -f "$MEMORY_CORE_MARKER" ] \
        || fail 'a skipped none-slot restore dropped the marker'
    rm -f -- "$OPENCLAW_CFG_UNDER_TEST" "$MEMORY_CORE_MARKER"
    echo 'PASS: uninstall reads slots.memory=none from openclaw.json when the CLI cannot answer'
fi
