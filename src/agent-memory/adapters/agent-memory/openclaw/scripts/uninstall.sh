...
#
# That includes the choice to run with no memory plugin at all. OpenClaw
# documents plugins.slots.memory = "none" as an explicit "disable memory
# plugins" value, not as an unset slot
# (https://docs.openclaw.ai/gateway/config-extensions), and `plugins enable`
# answers it with `Exclusive slot "memory" switched from "none" to
# "memory-core".` — so reading "none" as "nobody owns the slot" re-enables a
# backend the operator deliberately turned off, which is the same undo with no
# third-party owner to point at.
#
...
#
# Only a positively identified operator choice blocks the restore. An empty or
...
# an unwanted restore costs one `plugins disable`. "none" is not that case: it
# is an answer, and the operator's.
MEMORY_SLOT_KEY="plugins.slots.memory"
MEMORY_SLOT_OWNER=""
MEMORY_SLOT_TAKEN_BY=""
MEMORY_SLOT_DISABLED_BY_OPERATOR=0
...
case "$MEMORY_SLOT_OWNER" in
    # Nothing identified as an operator choice: no owner at all, a host that
    # renders "unset" as a bare word, this plugin's own entry, or memory-core
    # itself. Every one of them leaves the restore as the safe default. "none"
    # is pointedly absent from this list — see the slot-owner note above.
    ''|null|nil|undefined|nan|'(empty)'|"$PLUGIN_ID"|"$MEMORY_CORE_ID") ;;
    # One arm covers none / NONE / "none" / `plugins.slots.memory=none`:
    # slot_value() already lower-cased and stripped the rendering.
    none)
        RESTORE_MEMORY_CORE=0
        MEMORY_SLOT_DISABLED_BY_OPERATOR=1
        ;;
    *)
        RESTORE_MEMORY_CORE=0
...
elif [ "$MEMORY_SLOT_DISABLED_BY_OPERATOR" = "1" ]; then
    # The same deliberate skip for the operator who chose no memory backend at
    # all. The marker is kept for the same reason too: it still records that
    # memory-core is disabled *because of an anolisa install*, and dropping it
    # would leave a host that later moves ${MEMORY_SLOT_KEY} off 'none' with no
    # memory backend and no record of why.
    echo "[${COMPONENT}] Left ${MEMORY_CORE_ID} disabled: ${MEMORY_SLOT_KEY} is set to 'none', which"
    echo "[${COMPONENT}]       OpenClaw reads as an explicit choice to run with no memory plugin at all."
    echo "[${COMPONENT}]       '${OPENCLAW_BIN} plugins enable ${MEMORY_CORE_ID}' re-runs OpenClaw's exclusive slot"
    echo "[${COMPONENT}]       selection, so it would switch the memory slot from 'none' back to ${MEMORY_CORE_ID}"
    echo "[${COMPONENT}]       and undo that choice. Run it yourself if that is what you want, then delete"
    echo "[${COMPONENT}]       ${MEMORY_CORE_MARKER}."
fi
