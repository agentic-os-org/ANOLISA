...
impl Fixture {
    fn new() -> Self {
        ...
        let (worker_stop, worker_mode, key, physical) =
            (stop.clone(), mode.clone(), secret.clone(), live.clone());
    ...
    fn service(&self) -> Arc<SkillGuardService> {
...
fn respond_control(stream: UnixStream, key: &Secret, physical: &Path, mode: u8) {
    stream
    ...
    let metadata = fs::metadata(physical.join("demo")).unwrap();
    let mut response = json!({"schemaVersion":"1","ok":true,"result":{"managed":true,"canonicalSkillDir":request["canonicalSkillDir"],"skillId":"demo","relativeSkillDir":"demo","liveSkillDir":physical.join("demo"),"transport":"shared_path","identity":{"device":metadata.dev(),"inode":metadata.ino()}}});
    match mode {
...
fn rotation_commands_resolve_current_inodes_and_retain_failed_intents() {
    for command in [
        ...
        let mut handler = crate::skill_guard::SkillGuardHandler::new(service.clone(), finalizer);
...
async fn public_socket_authenticates_notify_and_keeps_v2_requests_separate() {
    ...
    let dispatcher = Arc::new(
        ...
        .with_skill_guard(service.clone(), finalizer)
    ...
    let status = ordinary(
        &socket,
        json!({"method":"action.skill_guard","params":{"command":"status"}}),
    )
    .await;
...
async fn assert_restart_and_failed_mapping(
    ...
    let status = ordinary(
        socket,
        json!({"method":"action.skill_guard","params":{"command":"status","verbose":true}}),
    )
    .await;
