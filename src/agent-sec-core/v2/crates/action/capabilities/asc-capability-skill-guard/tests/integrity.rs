...
use asc_capability_skill_guard::{
    GuardConfig, HashDiff, KeyStore, Manifest, SkillIdentity, hash_tree,
};
...
fn canonical_identity_is_lexical_and_validated_after_json_decode() {
    ...
    let config = GuardConfig {
        state_dir: "/var/lib/agent-sec/skillguard".into(),
...
fn findings_cannot_be_hidden_by_a_lower_scanner_summary() {
    ...
    record.scans[0].status = asc_capability_skill_guard::ScanStatus::Warn;
    record.scan_status = asc_capability_skill_guard::ScanStatus::Warn;
    key.sign_manifest(&mut record).unwrap();
}
...
fn corrupt_or_permissive_keys_are_not_automatically_replaced() {
    ...
    assert!(matches!(
        store.load(),
        Err(asc_capability_skill_guard::GuardError::Invalid(_))
    ));
}
