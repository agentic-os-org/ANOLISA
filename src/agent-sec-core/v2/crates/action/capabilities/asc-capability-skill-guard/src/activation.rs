...
use crate::{
    DecisionAction, GuardError, Manifest, ScanStatus, SigningIdentity, SkillRoot, UserDecision,
...
fn choose(
    ...
    root: &SkillRoot,
    deadline: Instant,
) -> Result<Choice, GuardError> {
    let mut records = Vec::new();
    for id in ledger.ids(deadline)?.iter().rev() {
        check_deadline(deadline)?;
        records.push(
            ledger
                .version(id, key, &root.identity, deadline)
                .ok()
                .filter(|manifest| ledger.snapshot(manifest, deadline).is_ok()),
        );
    }
    check_deadline(deadline)?;
    ...
    for record in &records {
        ...
        return Ok(Choice::Decision(manifest.clone()));
    }
    Ok(Choice::Policy(records.into_iter().flatten().find(|m| {
        matches!(m.scan_status, ScanStatus::Pass | ScanStatus::Warn)
    })))
}
...
fn publish_with(
    ...
    summary: &Value,
    deadline: Instant,
    set_xattr: impl FnOnce(&[u8]) -> Result<(), GuardError>,
) -> Value {
...
fn write_pending(ledger: &Ledger, root: &SkillRoot, deadline: Instant) -> Result<(), GuardError> {
...
mod tests {
    ...
    fn xattr_failure_preserves_committed_decision_and_reports_retry() {
        ...
        let result = publish_with(&ledger, &directory, &root, &selected, deadline(), |_| {
            Err(GuardError::Integrity("injected xattr failure".into()))
        });
