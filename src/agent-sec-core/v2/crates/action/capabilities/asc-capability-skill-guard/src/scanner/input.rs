...
impl ScanTree {
    pub fn open(root: &Path, deadline: Instant) -> Result<Self, GuardError> {
    ...
    fn walk(
        ...
        for item in Dir::read_from(directory).map_err(|e| io_error(prefix, e))? {
            ...
            let name = item
                .file_name()
                .to_str()
                .map_err(|_| GuardError::Invalid("non-UTF-8 Skill path".into()))?;
            if !matches!(name, "." | "..") {
    ...
    fn open_entry(&self, entry: &Entry) -> Result<File, GuardError> {
        let mut parent = self
            .directory
            .try_clone()
    ...
    pub fn read_prefix(&self, entry: &Entry, deadline: Instant) -> Result<Vec<u8>, GuardError> {
        check_deadline(deadline)?;
