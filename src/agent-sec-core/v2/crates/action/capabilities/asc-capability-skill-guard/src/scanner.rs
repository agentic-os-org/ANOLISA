...
use crate::{Finding, GuardError, ScanEntry, ScanStatus, check_deadline};
...
impl ScannerRegistry {
    ...
    pub fn scan(
        ...
        names: Option<&[String]>,
        deadline: Instant,
    ) -> Result<Vec<ScanEntry>, GuardError> {
    ...
    pub fn parse_external(
        ...
        scanner: &str,
        value: &Value,
    ) -> Result<ParsedFindings, GuardError> {
...
fn static_limit(scanner: &ScannerConfig) -> Result<u64, GuardError> {
    scanner
        .options
        .get("maxFileBytes")
        .map_or(Ok(1_000_000), |v| {
            v.as_u64().filter(|n| *n > 0).ok_or_else(|| {
                GuardError::Invalid("static maxFileBytes must be a positive integer".into())
            })
        })
}
...
pub fn validate_name(name: &str) -> Result<(), GuardError> {
    ...
    if name.trim().is_empty() {
        return Err(GuardError::Invalid("scanner name is empty".into()));
    }
    Ok(())
}

pub(crate) fn requested_names(names: Option<&[String]>) -> Result<Vec<String>, GuardError> {
...
fn parse_findings(value: &Value) -> Result<ParsedFindings, GuardError> {
    let items = value
        .as_array()
        .or_else(|| value.get("findings").and_then(Value::as_array))
        .ok_or_else(|| {
            GuardError::Invalid("expected a JSON array or an object with a 'findings' key".into())
        })?;
    ...
    for (index, item) in items.iter().enumerate() {
        ...
        let file = item
            ...
            .map(|v| {
                v.as_str()
                    .map(String::from)
                    .ok_or_else(|| GuardError::Invalid("finding file must be a string".into()))
            })
            .transpose()?;
        let line = item
            .get("line")
            ...
            .map(|v| {
                v.as_u64()
                    ...
                    .ok_or_else(|| GuardError::Invalid("finding line must be nonnegative".into()))
            })
            .transpose()?;
