...
use crate::{GuardError, Manifest, SigningIdentity, SkillIdentity, check_deadline};
use content::Content;
...
impl Ledger {
    pub fn open(root: &Directory, create: bool) -> Result<Option<Self>, GuardError> {
    ...
    pub fn ids(&self, deadline: Instant) -> Result<BTreeSet<String>, GuardError> {
        Ok(self
            .versions
            .names(deadline)?
            .into_iter()
    ...
    pub fn version(
        ...
        deadline: Instant,
    ) -> Result<Manifest, GuardError> {
        if !valid_version(id) {
            return Err(GuardError::Invalid("invalid version selector".into()));
        }
    ...
    pub fn snapshot(&self, manifest: &Manifest, deadline: Instant) -> Result<Content, GuardError> {
        let directory = self
            .versions
    ...
    pub fn newest(
        ...
        snapshots: bool,
        deadline: Instant,
    ) -> Result<Option<Manifest>, GuardError> {
    ...
    pub fn next_id(
        ...
        let start = match previous {
            Some(m) => {
                m.version_id[1..]
                    .parse::<u32>()
                    .map_err(|_| GuardError::Integrity("invalid predecessor ID".into()))?
                    + 1
            }
            None => 1,
        };
        ...
        Err(GuardError::Invalid("version ID space exhausted".into()))
    }
    ...
    pub fn commit(
        ...
        new_version: bool,
        deadline: Instant,
        before_publish: impl FnOnce() -> Result<(), GuardError>,
    ) -> Result<(), GuardError> {
        let bytes = serde_json::to_vec(manifest)?;
        if u64::try_from(bytes.len()).unwrap_or(u64::MAX) > MAX_RECORD_BYTES {
            return Err(GuardError::Invalid("signed record exceeds 8 MiB".into()));
        }
    ...
    pub fn audit(
        ...
        for id in &ids {
            ...
            match self.version(id, key, identity, deadline) {
                Ok(manifest) => {
                    ...
                    authenticated.insert(id.clone(), manifest);
                }
                Err(_) => errors.push(
        ...
        for (id, manifest) in &authenticated {
            if let Some(parent_id) = &manifest.previous_version_id {
                let parent_signature = authenticated
                    .get(parent_id)
                    .and_then(|p| p.signature.as_ref())
                    .map(|s| &s.value);
                if parent_signature.is_none()
                    || parent_signature != manifest.previous_manifest_signature.as_ref()
                {
