...
/// Daemon-owned authentication resources and worker using the shared application service.
...
impl SkillFsBridge {
    ...
    pub fn prepare(config: SkillFsConfig) -> Result<Self, SkillFsError> {
        ...
        Ok(Self {
            ...
            queue: Arc::new(worker::Queue::default()),
    ...
    /// Starts the worker once, after recovery and before public socket admission.
    ///
    /// # Errors
    /// Rejects repeated startup, poisoned state and thread creation failures.
    pub fn start_worker(&self, application: Arc<ActionService>) -> Result<(), SkillFsError> {
        let mut worker = self
            .worker
            .lock()
        ...
        *worker = Some(worker::start(self.queue.clone(), application)?);
        Ok(())
    }
