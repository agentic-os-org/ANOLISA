...
--skillguard-config selects a root-owned JSON configuration file.\n\
...
pub struct Cli {
    ...
    /// Optional root-owned `SkillGuard` settings file.
    pub skillguard_config: Option<PathBuf>,
}
...
impl Cli {
        ...
        T: Into<OsString>,
    {
        ...
        let mut skillguard_config = None;
        ...
        while let Some(argument) = arguments.next() {
            ...
            if argument == OsStr::new("--skillguard-config") {
                let path = arguments
                    .next()
                    .map(PathBuf::from)
                    .ok_or(CliError::InvalidSkillGuardConfig)?;
                if skillguard_config.is_some() || !path.is_absolute() {
                    return Err(CliError::InvalidSkillGuardConfig);
                }
                skillguard_config = Some(path);
                continue;
            }
...
pub enum CliError {
    ...
    #[error("--skillguard-config requires one absolute path")]
    InvalidSkillGuardConfig,
