...
fn configured_command(directory: &Path) -> Command {
    // Process tests never open the machine's production SkillGuard key store or configuration.
    let config = directory.join("skillguard.json");
    std::fs::write(
        &config,
    ...
    command.arg("--skillguard-config").arg(config);
    command
}
...
async fn rejected_without_root(running: &mut RunningBinary) -> bool {
    ...
    assert!(
        stderr.contains("SkillGuard system daemon must run as root"),
        "{stderr}"
    );
