...
impl SkillLedgerCommand {
    pub(crate) fn request(&self) -> Result<DaemonRequest, InputError> {
        let params = match self {
            ...
            Self::Decide {
                ...
                if *clear == action.is_some() {
                    return Err(InputError::SkillGuard(
        ...
        Ok(DaemonRequest {
            method: method::ACTION_SKILL_GUARD.into(),
            params,
        })
    }
...
fn absolute(path: &Path) -> Result<PathBuf, InputError> {
    let text = path
        .to_str()
        .ok_or_else(|| InputError::SkillGuard("Skill paths must be UTF-8".into()))?;
    let path = if text == "~" || text.starts_with("~/") {
        let home = std::env::var_os("HOME").ok_or_else(|| {
            InputError::SkillGuard("HOME is unavailable for path expansion".into())
        })?;
