...
实现以 main 的公共生命周期和扁平化 crate 结构为基础（`76f8314`，包含 PR #3300 与 #3319）。
...
不记录原文、findings、完整脱敏文本或规则内容。trace 归一化结果同时传递业务关联字段和
`ActionAttribution.agent_name`，保留原有审计元数据；身份仍以 UDS peer 为准。
本轮不增加 OTel 信封或第二套传播机制。
