...
The implementation uses the shared lifecycle and flat crate layout on main
(`76f8314`, including PRs #3300 and #3319). The phase-1 diagram describes the
...
findings, redacted text or rule contents. Trace normalization supplies both business
correlation and `ActionAttribution.agent_name`; existing audit metadata is retained.
Kernel peer identity remains authoritative. This adaptation adds no OTel envelope
or second propagation mechanism.
