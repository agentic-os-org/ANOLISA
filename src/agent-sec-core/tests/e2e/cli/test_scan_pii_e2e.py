...
def test_bundled_pii_skill_commands(mode: str, tmp_path: Path) -> None:
    ...
    for index, command in enumerate(commands):
        ...
        args = [
            str(input_path) if arg == "/absolute/path/to/input.txt" else arg for arg in argv[1:]
        ]
        result = _run_cli(
            mode,
            *args,
...
@pytest.mark.parametrize("mode", _MODES)
def test_scan_pii_filters_email_and_jwt_false_positives(mode: str, tmp_path: Path) -> None:
    text = (
    ...
    assert {finding["metadata"]["context"] for finding in detailed_data["findings"]} == {
        "remote_identity",
        "reserved_domain",
    }
...
@pytest.mark.parametrize("mode", _MODES)
def test_scan_pii_stdin_max_bytes_truncates_before_scan(mode: str, tmp_path: Path) -> None:
...
@pytest.mark.parametrize("mode", _MODES)
def test_scan_pii_raw_evidence_stays_out_of_security_events(mode: str, tmp_path: Path) -> None:
    ...
    assert all("raw_evidence" not in finding for finding in details["result"]["findings"])
