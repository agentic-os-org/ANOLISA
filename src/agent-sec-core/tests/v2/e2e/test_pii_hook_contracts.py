...
def _audit(environment, source, verdict=None):
    ...
    calls = {
        call["pid"] for call in _lines(directory / "calls.jsonl") if call["command"] == "scan-pii"
    }
    for event in events:
        assert event["category"] == "pii_scan"
        assert event["pid"] in calls, "audit PID must belong to the exec'd Rust subprocess"
...
@pytest.mark.parametrize("host", HOSTS)
@pytest.mark.parametrize("event", EVENTS)
def test_standalone_scanner_failure_preserves_fail_open(host, event, hook_environment, monkeypatch):
    monkeypatch.setenv("PII_CHECKER_MODE", "block")
    monkeypatch.setenv("AGENT_SEC_DAEMON_SOCKET", str(hook_environment[1] / "missing.sock"))
    output = _run([sys.executable, str(_asset(host) / "pii_checker_hook.py")], _payload(event))
    assert output in ({}, {"decision": "allow"})
...
def test_dense_report_blocks_with_complete_finding_count(host, hook_environment, monkeypatch):
    monkeypatch.setenv("PII_CHECKER_MODE", "block")
...
def test_hermes_native_pii_contract(policy, hook, fields, source, hook_environment, monkeypatch):
    monkeypatch.setenv("PII_CHECKER_MODE", policy)
...
def test_openclaw_native_pii_contract(policy, hook, fields, source, hook_environment, monkeypatch):
    monkeypatch.setenv("PII_CHECKER_MODE", policy)
...
def test_native_scanner_failure_preserves_fail_open(host, hook_environment, monkeypatch):
    monkeypatch.setenv("PII_CHECKER_MODE", "block")
    monkeypatch.setenv("AGENT_SEC_DAEMON_SOCKET", str(hook_environment[1] / "missing.sock"))
    if host == "hermes":
        ...
        assert (
            _openclaw("before_tool_call", {"toolName": "read", "params": {"content": TEXT}})[
                "result"
            ]
            is None
        )
