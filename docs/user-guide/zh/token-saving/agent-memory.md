...
服务关闭时自动从会话审计日志中提取原子事实（`mem_consolidate`），使用 6 条启发式规则（零 LLM 调用）识别高频路径、重复编辑、失败与提升等行为特征并持久化为结构化记忆。检索词不会写入审计日志（仅记录检索模式与检索词长度），也没有任何事实会回引这个长度：情景记忆只记录执行过一次 `bm25` 检索，而不记录检索了什么。也可通过 `mem_consolidate` 工具手动触发。含情景记忆提取与冲突检测（BM25 阈值）。
