...
These prompts are not blind bug reports: 80% name exactly one source file and about
one in five reproduces ≥75% of the gold patch's own added lines. That **suppresses
the effect being measured** (a compressor has little to compress when the prompt is
